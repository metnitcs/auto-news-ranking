...
export async function POST() {
    try {
        ...
        if (!analyzedNews || analyzedNews.length === 0) {
            return NextResponse.json({ message: "No analyzed news found for today" });
        }
        ...
        const enrichedRankedList = rankingResult.ranked.map((rankedItem: any) => {
            ...
            return {
                ...
                impact_score: original?.scores.impact || 0,
                social_trend_score: original?.scores.trend || 0
            };
        });
        ...
        return NextResponse.json({
            success: true,
            date: todayStr,
            count: itemsToRank.length,
            result: rankingResult
        });

    } catch (error) {
        console.error("Ranking Engine Failed:", error);

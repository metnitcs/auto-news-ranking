...
export async function POST() {
    try {
        ...
        // Ideally select news_summary where id not in news_analysis
        // 1. Find summaries not yet analyzed
        ...
        if (summaries.length === 0) {
            return NextResponse.json({ message: "No summaries found" });
        }

        // Check which ones are already analyzed
        // FOR DEBUGGING: We want to FORCE re-analyze even if exists, to fix the "5" scores.
        // So we skip the filtering step.
        // const summaryIds = summaries.map(s => s.id);
        // const { data: analyzed, error: checkError } = await supabase
        //     .from('news_analysis')
        //     .select('id')
        //     .in('id', summaryIds);

        // const analyzedIds = new Set(analyzed?.map((a: any) => a.id) || []);

        // Filter out already analyzed
        // const pendingAnalysis = summaries.filter(s => !analyzedIds.has(s.id));

        // FORCE RUN ON ALL FETCHED SUMMARIES
        const pendingAnalysis = summaries;

        if (pendingAnalysis.length === 0) {
            return NextResponse.json({ message: "No pending news to analyze" });
        }

        // 1.5 Fetch Meta manually (Robustness fix)
        // summaryIds already defined above
